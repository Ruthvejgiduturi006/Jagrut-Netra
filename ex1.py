    ...
    while cap.isOpened():
        ...
        for cnt in contours:
            ...
            for oid, data in loitering_tracker.items():
                if distance((cx, cy), data['position']) < 50:
                    object_id = oid
                    break

            if object_id is None:
                object_id = len(loitering_tracker)
                loitering_tracker[object_id] = {'position': (cx, cy), 'start_time': current_time}
            else:
                loitering_tracker[object_id]['position'] = (cx, cy)

            duration = current_time - loitering_tracker[object_id]['start_time']
            if duration >= loiter_threshold:
                cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 255, 0), 2)
                cv2.putText(frame, "Loitering Detected", (x, y - 10),
